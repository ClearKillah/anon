...
async def delete_pin_message(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    """Удаление сообщения о закреплении с использованием сохраненного ID."""
    try:
        # Сначала пробуем удалить по сохраненному ID
        if user_id in PIN_MESSAGE_IDS:
            message_id = PIN_MESSAGE_IDS[user_id]
            try:
                await context.bot.delete_message(
                    chat_id=user_id,
                    message_id=message_id
                )
                logger.info(f"Удалено сообщение о закреплении {message_id} для пользователя {user_id}")
                del PIN_MESSAGE_IDS[user_id]
            except Exception as e:
                if "Message to delete not found" in str(e):
                    # Сообщение уже удалено, очищаем ID
                    del PIN_MESSAGE_IDS[user_id]
                else:
                    logger.error(f"Ошибка удаления сообщения о закреплении по ID: {e}")
        
        # Проактивный поиск сообщений о закреплении
        try:
            # Получаем последние сообщения для поиска уведомлений о закреплении
            ...
            for message in messages:
                if message and (
                    message.text and any(text in message.text.lower() for text in ["закрепил", "закреплено", "pinned"]) or
                    message.type == "pinned_message"
                ):
                    try:
                        await context.bot.delete_message(
                            chat_id=user_id,
                        ...
                        logger.info(f"Удалено найденное сообщение о закреплении {message.message_id}")
                    except Exception as e:
                        if "Message to delete not found" not in str(e):
                            logger.error(f"Ошибка удаления найденного сообщения о закреплении: {e}")
                        
        except Exception as e:
            logger.error(f"Ошибка при проактивном поиске сообщений о закреплении: {e}")
            
    except Exception as e:
        logger.error(f"Ошибка в delete_pin_message: {e}")
...
async def stop_chat(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        # Delete pin notification messages first
        if user_id in PIN_MESSAGE_IDS:
            try:
                await context.bot.delete_message(chat_id=user_id, message_id=PIN_MESSAGE_IDS[user_id])
                del PIN_MESSAGE_IDS[user_id]
            except Exception as e:
                logger.error(f"Error deleting user's pin notification: {e}")
                
        if partner_id in PIN_MESSAGE_IDS:
            try:
                await context.bot.delete_message(chat_id=partner_id, message_id=PIN_MESSAGE_IDS[partner_id])
                del PIN_MESSAGE_IDS[partner_id]
            except Exception as e:
                logger.error(f"Error deleting partner's pin notification: {e}")
...
async def stop_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    active_chat = await db.get_active_chat(user_id)
    if not active_chat:
        # Используем основное сообщение вместо reply
        keyboard = [[InlineKeyboardButton("Начать поиск", callback_data="search_chat")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    try:
        ...
        logger.error(f"Error deleting stop command after execution: {e}")

async def pin_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Закрепление сообщения, на которое ответили."""
    ...
    try:
        # Сохраняем оригинальное сообщение команды для последующего удаления
        command_message = update.message
        command_message_id = command_message.message_id
        
        # Закрепляем сообщение без уведомлений
        message_to_pin = update.message.reply_to_message
        await message_to_pin.pin(disable_notification=True)
        
        # Ждем появления уведомления о закреплении
        await asyncio.sleep(1)
        
        # Пытаемся удалить уведомление несколько раз с экспоненциальной задержкой
        for attempt in range(5):
            try:
                # Сначала удаляем оригинальную команду
                await context.bot.delete_message(chat_id=user_id, message_id=command_message_id)
            except Exception as e:
                if "Message to delete not found" not in str(e):
                    logger.error(f"Ошибка удаления сообщения команды: {e}")
            
            # Пытаемся удалить уведомление о закреплении
            await delete_pin_message(user_id, context)
        ...
        # Обновляем основное сообщение партнера с уведомлением
        await update_main_message(
            partner_id,
            context,
        ...
        logger.error(f"Ошибка в pin_message: {e}")
        try:
            await update.message.reply_text("Произошла ошибка при закреплении сообщения. Попробуйте еще раз.")
        except Exception:
            pass
...
async def unpin_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not active_chat:
        ...
        try:
            ...
            logger.error(f"Error deleting unpin command: {e}")
    ...
    try:
        ...
        if chat.pinned_message:
            ...
            # Update main message with a notification temporarily
            original_message_user = MAIN_MESSAGE_IDS.get(user_id, None)
            original_message_partner = MAIN_MESSAGE_IDS.get(partner_id, None)
            ...
            # Update messages with notification
            await update_main_message(
                user_id,
                context,
            ...
            await update_main_message(
                user_id,
                context,
                "Нет закрепленных сообщений. Вы в чате с собеседником.",
                reply_markup
            )
            ...
            try:
                ...
                logger.error(f"Error deleting unpin command after no pinned messages: {e}")
        ...
        await update_main_message(
            user_id,
            context,
            "Не удалось открепить сообщение. Вы в чате с собеседником.",
            reply_markup
        )
        ...
        try:
            ...
            logger.error(f"Error deleting unpin command after error: {e}")
...
async def clear_history(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not active_chat:
        ...
        try:
            ...
            logger.error(f"Error deleting clear command: {e}")
    ...
    try:
        ...
        await update_main_message(
            user_id,
            context,
            "Не удалось очистить историю чата. Вы в чате с собеседником.",
            reply_markup
        )
        ...
        try:
            ...
            logger.error(f"Error deleting clear command after error: {e}")

async def handle_service_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Обработка сервисных сообщений, таких как уведомления о закреплении."""
    if not update.message:
        return
        
    user_id = update.effective_chat.id
    
    # Проверяем, является ли это уведомлением о закреплении или системным сообщением
    is_pinned_update = (
        update.message.type == "pinned_message" or
        (update.message.text and any(text in update.message.text.lower() for text in ["закрепил", "закреплено", "pinned"]))
    )
    
    is_system_message = (
        update.message.text and
        any(text in update.message.text.lower() for text in ["системное", "system", "notification"])
    )
    
    is_pin_message = (
        update.message.text and
        any(text in update.message.text.lower() for text in ["закреплено", "pinned"])
    )
    
    # Если это уведомление о закреплении или похожее системное сообщение
    if is_pinned_update or is_system_message or is_pin_message:
        try:
            logger.info(f"Найдено уведомление о закреплении: {update.message.text or 'Нет текста'}, message_id: {update.message.message_id}")
            
            # Сохраняем ID сообщения перед попыткой удаления
            if update.effective_chat:
                PIN_MESSAGE_IDS[user_id] = update.message.message_id
            
            # Пытаемся удалить уведомление
            try:
                await update.message.delete()
                logger.info(f"Успешно удалено уведомление о закреплении {update.message.message_id}")
            except Exception as e:
                if "Message to delete not found" not in str(e):
                    logger.error(f"Ошибка удаления уведомления о закреплении: {e}")
                else:
                    # Сообщение уже удалено, очищаем ID
                    if user_id in PIN_MESSAGE_IDS:
                        del PIN_MESSAGE_IDS[user_id]
            ...
            logger.error(f"Ошибка обработки сервисного сообщения: {e}")
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(
        filters.TEXT & ~filters.COMMAND & filters.ChatType.PRIVATE & filters.Regex(r'(закреплено|pinned|message)'),
        handle_service_message
    ))
